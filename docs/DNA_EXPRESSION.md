...
5. **多细胞 v2 染色体遗传**（`src/genetics/genome.js`）：96 位拆为 **12 对 × 8 位** 同源染色体；减数分裂每对随机分离一条进入配子；受精为卵单倍体 + 精单倍体合并为二倍体；**第 12 对为性染色体**（首位 `3` 为 Y，否则 X）；合子父源 Y → `pairMorph A`，否则 `pairMorph B`。表达时先按位合并双等位（`max` 显性简化），再对 96 位表达串做 Z1–Z6 哈希。
