...
5. **多细胞 v2 染色体遗传**（`src/genetics/genome.js`）：96 位拆为 **12 对 × 8 位** 同源染色体（对 1–2→Z1 … 对 11–12→Z6）；减数分裂前可按概率在同源间 **交叉互换**（断点 `互换@n`），再随机分离并记录同源来源；受精为卵单倍体 + 精单倍体合并为二倍体；**第 12 对为性染色体**（首位 `3` 为 Y，否则 X）；合子父源 Y → `pairMorph A`，否则 `pairMorph B`。表达：**Z3/Z4 偏共显性**、**Z1/Z6 偏强显性**、其余默认；再对 96 位表达串做 Z1–Z6 哈希。族谱登记压缩 `inheritSummary`；`[MEI]`/`[DCK]` 日志含 `cross 对:位点`。
