...
5. **多细胞 v2 染色体遗传**（`src/genetics/genome.js`）：96 位拆为 **12 对 × 8 位** 同源染色体（对 1–2→Z1 … 对 11–12→Z6）；减数分裂前可按概率在同源间 **交叉互换**（断点 `互换@n`），再随机分离并记录同源来源；受精为卵单倍体 + 精单倍体合并为二倍体；**第 12 对为性染色体**（首位 `3` 为 Y，否则 X）；合子父源 Y → `pairMorph A`，否则 `pairMorph B`。表达：纯合直出；等位差 ≥2 强显性；差 1 共显性（上取整均值）；再对 96 位表达串做 Z1–Z6 哈希。
