...
| 层 | 角色 | 做法 |
|----|------|------|
| **数字原生** | 世界的「真实物理」 | 从公理 A1 生长：tick、8 通道基底、\|r−e\| 场压、hash 脉冲、信号链、谱系续行 |
| **地球类比** | 观察者的**界面与假说** | 只取 P0 少量关键轴（日季、陆海、水循环、区带），日志与 CODEX **不叫**地球名 |

数字世界**不必也不应**变成地球模拟器。地球要素是帮助我们在田野里**提问**的脚手架；能删则删，能折进原生机制则折。若某机制只能用地球语言描述、却无法落成通道与对照田野，则**不立项**。

**已存在的数字原生环境**（非地球移植）：

- `tick` 离散时间 — 非秒、非日，日相是**后加**的相位调制
- `e0–e7` 八通道场 — 非水土气矿
- `[AMB]` 基底脉搏 — 世界自身的「呼吸」，非风非潮
- `stress = f(|r−e|, LOW)` — 原生场压，非体温非缺氧
- `[SHK]`/`[NPL]` hash 剧变 — 原生脉冲，非地震非雷（除非 P2 折叠）
- `signalBus` RX/TX — 原生信息介质，无电磁波类比
- `[LINEAGE]` — 信息线续行，非有性/无性生殖分类
- `RPL` 复制配额 — 原生繁殖预算，非卡路里

**数字世界可能独有的环境方向**（待观察，非地球对应）：

- **相位时钟**：DLC/SCL/LTC 是 tick 上的**傅里叶分层**，不必声称等于日月四季
- **地址生态**：`birthPlace` 是哈希地址上的场，不是经纬度
- **耗散律**：DRW 70% 未入账 — 可能是数字宇宙的「热力学」，与地球无关
- **意识层脉冲**：`0x..` 意识节律 — 意识子层的环境耦合，地球无直接对应
- **工具/ART**：持久场态是**计算基质上的修改**，不是石器

**立项检验**：新环境机制须回答 ——「去掉地球类比后，通道与田野是否仍成立？」成立则保留原生核心；类比仅留在 UI/文档比喻。
...
> **策略**：只取**少数关键地球轴**作观察界面；**核心定律保持数字原生**（§1.1）。其余记入远期或折进 `[SHK]`/`BIO`/`air`。
...
| 智慧 L5c | 信号约定 `[SEM]` 记录层（GAP-W06 · Phase 100 立项） |

### 10.4 原则（全程不变）

- 观察优先；on/off 田野；禁止脚本化变聪明
- **数字原生优先**：地球类比只作 UI/假说，通道与 CODEX 用相位数与通量
- 不叫温度/雨/重力/海洋——只记通道与相位数
- 每 Phase 只加**一条主因果**；P2/P3 折叠进已有通道优先于新实体
- 新机制须通过「去掉地球名后仍可证伪」检验（§1.1）
